...
model_pos_train =  MCLICE(num_frame=receptive_field, num_joints=num_joints, in_chans=2, embed_dim_ratio=args.cs, depth=args.dep,
                          num_heads=8, mlp_ratio=2., qkv_bias=True, qk_scale=None, drop_path_rate=0.1)

model_pos =  MCLICE(num_frame=receptive_field, num_joints=num_joints, in_chans=2, embed_dim_ratio=args.cs, depth=args.dep,
...
if not args.evaluate:
    ...
    while epoch < args.epochs:
        ...
        for cameras_train, batch_3d, batch_2d in train_generator.next_epoch():
            ...
            Rleg_order = torch.LongTensor([0, 1, 2, 3]).cuda()
            Lleg_order = torch.LongTensor([0, 4, 5, 6]).cuda()
            Torso_order = torch.LongTensor([0, 7, 8, 9, 10]).cuda()
            Rarm_order = torch.LongTensor([8, 14, 15, 16]).cuda()
            Larm_order = torch.LongTensor([8, 11, 12, 13]).cuda()

            Rleg = torch.index_select(predicted_3d_pos, dim=2, index=Rleg_order).cuda()
            Lleg = torch.index_select(predicted_3d_pos, dim=2, index= Lleg_order).cuda()
            Torso = torch.index_select(predicted_3d_pos, dim=2, index=Torso_order).cuda()
            Rarm = torch.index_select(predicted_3d_pos, dim=2, index=Rarm_order).cuda()
            Larm = torch.index_select(predicted_3d_pos, dim=2, index=Larm_order).cuda()

            Rleg_limb_length = torch.norm((Rleg[:,:,1:,:] - Rleg[:,:,:-1,:]),dim=3)
            Lleg_limb_length = torch.norm((Lleg[:,:,1:,:] - Lleg[:,:,:-1,:]),dim=3)
            Torso_limb_length = torch.norm((Torso[:,:,1:,:] - Torso[:,:,:-1,:]),dim=3)
            Rarm_limb_length = torch.norm((Rarm[:,:,1:,:] - Rarm[:,:,:-1,:]),dim=3)
            Larm_limb_length = torch.norm((Larm[:,:,1:,:] - Larm[:,:,:-1,:]),dim=3)

            
            R_length = torch.cat((Rleg_limb_length,Rarm_limb_length),dim=2)
            L_length = torch.cat((Lleg_limb_length,Larm_limb_length),dim=2)
            symmetry_weight = torch.tensor([1, 1.75, 2.5, 1.25, 2.75, 4]).cuda()
            R_length = R_length * symmetry_weight
            L_length = L_length * symmetry_weight

            # Rhipbone, Rtight, Rcalf
            # Lhipbone, Ltight, Lcalfdim=2
            # down_spine, middle_spine, up_spine, neck
            # Rshoulder, RBigLimb, Rforearm
            # Lshoulder, LBigLimb, Lforearm
            
            temporal_weight = torch.tensor([1, 1.75, 2.5, 1, 1.75, 2.5, 1, 1, 1, 1.25, 1.25, 2.75, 4, 1.25, 2.75, 4]).cuda()
            Limb_length = torch.cat((Rleg_limb_length,Lleg_limb_length,Torso_limb_length,Rarm_limb_length,Larm_limb_length),dim=2)
            Limb_length = temporal_weight * Limb_length
            ...
            assert R_length.shape == L_length.shape
            loss_symmetry_limb = torch.mean(torch.norm(R_length-L_length,p=1,dim=2))
            loss_temporal_limb= torch.mean(torch.norm(Limb_length[:,1:,:] - Limb_length[:,:-1,:],p=1,dim=2))

            loss_total = loss_3d_pos + loss_diff + loss_temporal_limb + loss_symmetry_limb
